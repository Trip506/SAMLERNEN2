					...
					foreach ($intdate as &$aEvent){
						if ($objEvents->id == $aEvent['id']) {															
							if ($UserIsAdmin && (!$aEvent['disable_editing'])) {
								...
								if ($currentTime <= $aEvent['startTime']) {
									// Event is in the future
									if (((!$objCalendar->caledit_onlyUser) || ($aEvent['FE_User'] == $userID))
										&& (!$aEvent['disable_editing']) )  {									
											// user is allowed to edit the event
											$this->addEditLinks($aEvent, $strUrl);											
									}
								} else {
									...
									if (!$objCalendar->caledit_onlyFuture
										...
										&& (!$aEvent['disable_editing']) ) ) {
											// user is allowed to edit past events
											$this->addEditLinks($aEvent, $strUrl);
									}									
								}
							}						
