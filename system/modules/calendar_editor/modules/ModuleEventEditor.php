    ...
    */
    ...
    public function IsUserAllowedToEditCalendar($user, $objCalendar)
    {
        if ($objCalendar->AllowEdit) {
			...
			if (!$objCalendar->caledit_loginRequired) {
				// if no login is required, every registered user is allowed to edit
				return true;
			}
    ...
    public function checkUserEditRights($user, $eventID, $CurrentObjectData)
    {
        ...
        if ($this->IsUserAllowedToEditCalendar($user, $objCalendar)) {
			...
			$userIsAdmin = $this->IsUserAdminForCalendar($user, $objCalendar);			
			if (!$userIsAdmin && 
	...
	{		
		...
		if ((!$objCalendar->caledit_onlyFuture) || 
			($this->IsUserAdminForCalendar($this->User, $objCalendar))) {		
	...
	protected function HandleClone($currentEventObject) 
	{
		...
		for ($i = 1; $i <= 10; $i++) {
			...
			/// XXXX   ZUSÄTZLICHE ABFRAGE; OB ÜBERHAUPT WAS DRINSTEHT ???
    ...
    protected function compile()
    {
		...
		// what is this??? deprecated code???
		//$postStart = $this->Input->post('startdate');
		//if (!empty($postStart)) {
		//	$NewEventData['startDate'] = $postStart;
		//}
